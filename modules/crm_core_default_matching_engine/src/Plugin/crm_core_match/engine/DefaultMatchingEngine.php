...
class DefaultMatchingEngine extends MatchEngineBase {
  ...
  public function match(ContactInterface $contact) {
    ...
    $matcher = $this->configuration['plugin_config'];

    $fields = $contact->getFieldDefinitions();
    $results = array();
    foreach ($matcher->getSetting('rules') as $name => $rules) {
    ...
    foreach ($results as $id => $rule_matches) {
      ...
      if ($total_score >= $matcher->getSetting('threshold')) {
        $ids[] = $id;
      }
    }
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['threshold'] = array(
      ...
      '#size' => 28,
      '#required' => TRUE,
      '#default_value' => $matcher->getSetting('threshold') ?: '',
    );
...
EOF
    ...
    $form['return_order'] = array(
      ...
      '#default_value' => $matcher->getSetting('return_order') ?: '',
      '#options' => array(
...
EOF
    ...
    $form['strict'] = array(
      ...
      '#default_value' => $matcher->getSetting('strict') ?: '',
    );
    ...
    $contact_types = $this->entityManager->getStorage('crm_core_match')->loadMultiple();
    $fields = [];
    ...
    foreach ($fields as $field) {
      ...
      $rules = $matcher->getSetting('rules');
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    // @todo: Implement plugin config interface
